...
% 数据源（与论文表9/表10 口径一致）：
%   ZDT2 (bench 2): data/ablation_benchmark.mat（ZDT 行 TMAX=300）
%   DTLZ2(bench 5): data/ablation_benchmark.mat（DTLZ2 T=300 重跑，与表9 一致）
% 风格：ZDT2 对数轴，DTLZ2 线性轴
clc; close all;
root = fileparts(fileparts(mfilename('fullpath')));   % = MOALA/
addpath(root);

% 数据源: data/ablation_benchmark.mat（ZDT2 行） + data/ablation_benchmark.mat 的 DTLZ2 行
S  = load(fullfile(root,'data','ablation_benchmark.mat'));
R  = S.R;
algoNames = S.algoDefs(:,1);
% DTLZ2 (bench 5) 已在统一包中使用 T=300 重跑数据，与论文表9/表10 口径统一
nAlgo = size(R,2);

benchIdx   = [2 5];
benchTitle = {'(a) ZDT2 (bi-objective)', '(b) DTLZ2 (three-objective)'};
