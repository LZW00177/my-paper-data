...
% 数据源（统一包）:
%   data/ablation_benchmark.mat（= ablation_final_v2 的 5 个基准 + DTLZ2 T=300 重跑）
%   6 基准 IGD Friedman 排名; ZDT TMAX=300; DTLZ1/3 TMAX=2000; DTLZ2 TMAX=300
clc; close all;
root = fileparts(fileparts(mfilename('fullpath')));   % = MOALA/
addpath(root);

% 唯一干净数据源: data/ablation_benchmark.mat
F = load(fullfile(root,'data','ablation_benchmark.mat'));
